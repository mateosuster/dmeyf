...
karch_dataset  <- "./datasets/dataset_epic_v951_v2.csv.gz"
ksalida  <- "semillerio" 

kexperimento  <- NA
kscript         <- "s1431_superdata"


kcantidad_semillas  <- 50

#ATENCION
#aqui deben ir los mejores valores que salieron de la optimizacion bayesiana
x  <- list()
x$gleaf_size   <-  65.6022568240091
x$gnum_leaves  <-  0.171574331387622
x$learning_rate <-  0.116142482437193
x$feature_fraction <-  0.121972466840147
x$max_bin  <-  93
x$num_iterations  <-132   
x$pos_ratio  <- 0.0382532868034111
...
param_buenos  <- list( objective= "binary",
                       ...
                       lambda_l1= 8,
                       lambda_l2= 15,
                       bagging_fraction = 0.232813652883152,
                       path_smooth = 0.544713322810778,
