...
Pasemos ahora a una variable que podemos *suponer* que puede ser categórica. Tomemos `Visa_status` combinada con `Master_status`, aunque quizás sea más ordinal que categórica (esta no la elegimos ya desde un criterio no tan aleatorio).

```{r}
ds[,  `:=` (status_all = rowMeans(.SD, na.rm= T)), .SDcols =c("Visa_status","Master_status")   ]

ds_tarjeta_estado <- dcast(ds, status_all +  Visa_status + Master_status  ~ clase_ternaria, 
                        length, 
                        value.var = "clase_ternaria" )
...
ds_tarjeta_estado[, Visa_status := factor(Visa_status)]

ds_tarjeta_estado
```

Para tener una visión completa, graficamos la cantidad de clientes en cada una de la categorías de `Visa_status` y `Master_status`

```{r}
ggplot(ds_tarjeta_estado, aes(x=Visa_status, y=total)) +
...
ggplot(ds_tarjeta_estado, aes(x=Visa_status, y=ratio_baja2)) +
