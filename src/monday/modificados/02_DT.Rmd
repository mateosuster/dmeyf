...
<<<<<<< HEAD
    ...
    Si ponemos en los dos lados el peso de BAJA+2 con el resto de los casos, no hay forma de separar un conjunto tan grande de forma tal que sea positivo.
    Es imposible distribuir nuestros positivos de forma tal de generar ganancia en todos los espacios.
    No es posible separar un conjunto tan grande de forma tal que haya ganancia positiva en todos lados.
    Como se seteó la función de ganancia, obtener gcia a ambos lados no es posible.
    Pero donde llegamos a tener ganancia positiva, como es en cardinalidad (su tamañao) el otro intervalo?
=======
>>>>>>> e163358e57519885b62ad2aa4ba623c552cc78e0
...
-   ¿Cómo trata los árboles las variables con datos ausentes?
    ...
    rpart, una vez que el arbol corta una vble genera dos categorías y el NA puede ser considerado como una categoría.

-   Usualmente se suele cortar las variables en 2 intervalos

    -   ¿Se podría cortar en más intervalos?

        -   de poderse, es una búsqueda mucho más intensa y más compleja

    -   ¿Cuál sería el costo?

        -   aumentan la cantidad de cortes

    -   ¿Se obtendrían mejores resultados?

        -   sí, pero con arboles más pequeños. cortarlo de a dos no es malo, pero es más económico. aunque quizas es mejor tener un arbol un poco mas profundo con cortes de a dos
...
-   ¿Por qué es distinto al teórico?
  Porque no hay ninguno con dicha probabilidad

-   ¿Es nuestro `punto de corte` es igual de útil?
  Si, es mas intuitivo/ingenuo
...
-   ¿Qué representa la **curva ROC**?
  Pone al TPR (prob de que acertemos; probability of detection) frente al FPR (prob de mandar pero que eran falsos sobre la cantidad de negativos que existen; prob de una falsa alarma).  
  Pero para este problema no necesitamos un TPR (recall) más alto?? A la curva no le interesa tanto la proporción del target; incluso podría rebalancear la proporción del target y esta se vería casi inalterada. Lo que mide la curva ROC es cuanto puedo separar mis clases (cual es la prob de una clase negativa). Puedo obtener una buena imagen de mi problema independientemente de mi balanceo. Muestra en función de cuánto error yo estoy dispuesto a aceptar, cuántos ratio de acierto tengo. Ej: si yo no puedo tolerar mucho error, hay que aceptar que la tasa de acierto va a ser baja (TPR). 
  El fin de la curva es que te de una buena métrica que atraviese modelos. La relación de costos (o la medición de la gcia del negocio) se obtiene con una matriz de profits. 
  La curva ROC puede distorcionarse en función de los FP y TP que tengamos. En el espacio donde se encuentra la relación adecuada, será nuestro "punto de equilibrio".
...
-   ¿**AUC** es una métrica global o local?
    Es una métrica global pq considera todos los puntos de corte. Mide una métrica para todos los elementos. 
    Una métrica local, en cambio, es una que evalúa un sólo punto de corte.
    La curva ROC es la segunda métrica más importante.
    La primera es la ganancia del negocio (que es una métrica local)
    
    Una curva ROC con alta AUC representa un modelo que ordena bien. Pero que la curva ordene bien mucho no nos importa pq a nosotros nos interesan solo los pocos casos de BAJA+2 a los cuales les vamos a enviar el regalo. 
...
-   ¿Por qué se produce semejante dispersión?
    Uno de los efectos q puede solucionar esto es que las distribuciones de train y test sean similares.
    Como hacemos?
    Dejar de lado datos?
    No podemos.
    Es dificil atacar este problema en train y test.
...
-   ¿Cuál es la diferencia entre **test** y **validation**? validación suele usarse para seleecionar el modelo; y test solo para evaluar el modelo

**Una pregunta MUCHO más interesante**

-   Si entendemos las diferencias entre **test** y **validation** ... cuántas veces podemos usar el conjunto de **test** sin convertirlo en **validation**? una sola? podemos usar kaggle una sola vez?
