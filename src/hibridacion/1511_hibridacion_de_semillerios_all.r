...
arch1  <- "./kaggle/kaggle_E5019_meseta_E5019_s1431_v1_150_14000.csv"
arch2  <- "./kaggle/kaggle_E5024_meseta_E5024_s1431_rankeo_120_14000.csv"
arch3  <- "./kaggle/kaggle_E5025_meseta_E5025_s1431_rankeo_70_70_14000.csv"
arch4  <- "./kaggle/kaggle_E5028_meseta_E5028_s1431_superdata_50_14000.csv"


kexperimento <-  NA

kscript <-  "s1511_hibridacion_all_143x"
...
setorder( modelo4, numero_de_cliente )

modelo_nuevo  <- copy(  modelo1[  , c("numero_de_cliente"), with=FALSE ] )

modelo_nuevo[ , ranking  :=  150*modelo1$ranking +120*modelo2$ranking + 70*modelo3$ranking+50*modelo4$ranking ]   #ATENCION  el 200 es por las 200 semillas del modelo1, y el 70 por las 70 del modelo2

setorder( modelo_nuevo, ranking ) 

for(  corte  in seq( 9500, 14000, 500) ) #imprimo cortes en 10000, 11000, 12000, 13000, 14000 y 15000
{
